...
if (mainOptions.command === 'auto') {
  const mergeDefinitions = [
    ...
    { name: 'useAndroidFullPDF', type: Boolean, defaultValue: true },
...
(async () => {
  ...
  const schema = {
    properties: {
      ...
      webViewerServer: {
        ...
        ...globalPrompt,
        ask: () => {
          if (optimizeOptions.hasOwnProperty('usingWebViewerServer')) {
            return false;
          }
          return true;
        },
      ...
      useAndroidFullPDF: {
        description: `Do you need to support Android devices? [y/n]${RESET}`,
        ...globalPrompt,
        ask: () => prompt.history('xod') && prompt.history('xod').value === 'n' && prompt.history('webViewerServer').value === 'n' && prompt.history('useLeanPDF').value === 'y',
      },
      pdfnetProd: {
  ...
  prompt.get(schema, (err, result) => {
    ...
    if (mainOptions.command === 'auto') {
      ...
      useAndroidFullPDF = optimizeOptions.useAndroidFullPDF  ? 'y' : 'n';
    ...
    const filesToRelocate = [
      ...
      [path.resolve(__dirname, `../${wvFolder}/core/pdf/ResizableWorker.js`), `${resourceDir}/resource`],
    ...
    if (webViewerServer === 'n' && xod === 'n') {
      ...
      // If they do need the full API
        ...
        if (useAndroidFullPDF === 'n') {
          filesToDelete.push(
            path.resolve(__dirname, `../${wvFolder}/core/pdf/full/PDFNetCAndroidWasm.br.js.mem`),
            path.resolve(__dirname, `../${wvFolder}/core/pdf/full/PDFNetCAndroidWasm.br.wasm`),
            path.resolve(__dirname, `../${wvFolder}/core/pdf/full/PDFNetCAndroidWasm.gz.js.mem`),
          );
        }
